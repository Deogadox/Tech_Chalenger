...
Após a carga e organização dos dados no SQL Server, as informações serão utilizadas como fonte para o desenvolvimento do **relatório no Power BI**.
...
O Power BI será utilizado como camada de **Business Intelligence**, permitindo a exploração dos dados e a apresentação dos principais insights obtidos a partir do dataset.
...
Execute o script para realizar o download do dataset.

Os arquivos CSV serão armazenados na pasta configurada.

### 4. Configurar o SQL Server

Execute o script de criação das tabelas no banco:
...
```text
sql/sp_carga_olist.sql
```

no SQL Server.

Verifique se os caminhos dos arquivos na procedure correspondem ao diretório configurado no Python.
